...
export class HomeComponent implements OnInit {
  ...
  // facturasMasVencidasTopDiez: { name: string; value: number }[] = [];
  facturasMasVencidasTopDiez: {
    nombre: string;
    comprobante: string;
    saldo: number;
    vencimiento: string;
  ...
  cargarDataCompleta() {
    this.facturaService.getResumenCartera().subscribe((historico) => {
  ...
  getResumenCarteraPorMes(periodo: number) {
    this.facturaService.getResumenCarteraPorPeriodo(periodo).subscribe((data) => {
  ...
  graficaWorldPopulation() {
    ...
    this.worldPopulation = {
      ...
      xAxis: {
        ...
        splitNumber: 3,
        axisLabel: {
          formatter: (val: number) => `$ ${val.toLocaleString('es', { minimumFractionDigits: 0 })}`,
        },
      },
      yAxis: {
        type: 'category',
  ...
  filtrarRangoWP(key: string) {
    ...
    const categoriasVisibles =
      key === 'todos' ? this.todasCategorias : this.todasCategorias.filter((c) => c.key === key);
  ...
  get tasaMorosidadPorcentaje(): number {
    const periodo =
      this.mesHistoricoSeleccionado == 0 ? this.mesActual : this.mesHistoricoSeleccionado;
  ...
  get montoTotalHistoricoVencido(): number {
    ...
    const totalVencido = this.resumenCartera
      .filter((rc) => rc.periodo == periodo)
      .reduce((total, rc) => {
        return (
          total +
          rc.vencido0a30 +
  ...
  get montoTotalHistorico(): number {
    ...
    const totalHistorico = this.resumenCartera
      .filter((rc) => rc.periodo == periodo)
      .reduce((total, rc) => {
        return (
          total +
  ...
  getFacturasMasVencidasTopDiez() {
    this.facturaClienteService.getFacturasPorCobrarVencidasTopDiez().subscribe((data) => {
      this.facturasMasVencidasTopDiez = data.map((f: any) => ({
        nombre: f.nombre,
  ...
  graficaTopDiezVencidas() {
    const datos = this.facturasMasVencidasTopDiez;

    this.topDiezVencidasChart = {
      tooltip: {
        trigger: 'axis',
        axisPointer: { type: 'shadow' },
        formatter: (params: any) => {
      ...
      yAxis: {
        type: 'category',
        inverse: true,
        data: datos.map((f) => f.comprobante), // 👈 comprobante en lugar de nombre (es único)
        axisLabel: {
          fontSize: 10,
          width: 100,
          overflow: 'truncate',
          formatter: (val: string, index: number) => {
            // Muestra: "Cliente - FT-001"
            const factura = datos[index];
            return `${factura.nombre.substring(0, 12)}...`;
          },
        },
      },
      series: [
        {
          type: 'bar',
          data: datos.map((f) => f.diasVencido),
          itemStyle: {
            color: {
              type: 'linear',
              x: 0,
  ...
  getFacturasPorCobrarTopDiez() {
    this.facturaClienteService.getFacturasPorCobrarTopDiez().subscribe((data) => {
  ...
  get totalImporte(): number {
    return this.facturasPorCobrar.reduce((acumulador, factura) => acumulador + factura.importe, 0);
  }

  //FACTURAS POR COBRAR: SUMA DE TODOS LOS MONTOS DE SALDO
  get totalSaldo(): number {
    return this.facturasPorCobrar.reduce((acumulador, factura) => acumulador + factura.saldo, 0);
  }
  ...
  /** Seccion de clientes */
  getClientes() {
    return this.clienteService.getClientes().subscribe((data) => (this.clientes = data));
  }
  ...
  getClientesDeudoresPorVendedor(idVendedor: number) {
    this.clienteService
      .getDeudoresPorVendedor(idVendedor)
      .subscribe((data) => (this.clientesDeudores = data));
  }

  /** Seccion de Vendedor */
  getVendedor() {
    return this.vendedorService.getVendedor().subscribe((data) => (this.vendedores = data));
  }
  ...
  get facturasPorCobrarAgrupadas(): {
    nombre: string;
    totalImporte: number;
    totalSaldo: number;
